...
export default function ContactForm() {
  ...
  const sendEmail = (captchaValue) => {
    ...
    emailjs.send('default_service', 'template_nzx5ltk',  //insert your own email js names here (service id and template id)
      
    params,
    'user_P7DgoSns9xdSsRV2z8xmV', //API Key User ID
    )
      .then(({ status }) => {
        if (status === 200) {
          setFormSubmitted({ title: 'Message has been sent', paragraph: 'Forward South will be in contact with you soon.' });
        } else {
          setFormSubmitted({ title: 'Unexpected status code returned from EmailJS, try again later', paragraph: 'Please contact Forward south at xxxxxxx' });
        }
      }, (err) => {
        // eslint-disable-next-line no-console
        console.log(err);
        setFormSubmitted({ title: 'Error sending message, try again later', paragraph: 'Please contact Forward south at xxxxxxx' });
      });
  };

  return formSubmitted.title === '' ? (
    <div>
      <h3 className="text-lato text-2xl font-light text-white">Send us a message</h3>
      {!showCaptcha ? (
        ...
        <ReCAPTCHA
          sitekey={'a5d2d05f15c906a36bd30175848e8aeb'} //sitekey here (access token)
          onChange={sendEmail}
        />
      )}
    </div>
